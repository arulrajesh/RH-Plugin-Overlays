...
function build_nextup(leaderboard, display_type, meta, ddr_pilot_data, ddr_frequency_data, show_position=false, ddr_heat_colors={}) {
    ...
    for (var i in sortedLeaderboard) {
        ...
        let color_source = 'default'; // Track where color came from
        ...
        if (i === 0) {
            ...
            console.log('[DDR Overlays] Available heat colors:', Object.keys(ddr_heat_colors || {}).length > 0 ? JSON.stringify(ddr_heat_colors) : 'none');
            console.log('[DDR Overlays] Secondary colors from settings:', JSON.stringify(secondaryColors));
        }
